...
import { applyTabSort } from '../background/apply-tab-sort.js';
import {
  ensureChromeApi,
...
test('applyTabSort returns move counts after sorting tabs', { concurrency: false }, async () => {
  ...
  const result = await applyTabSort(1);
...
test('applyTabSort reports partial move failures', { concurrency: false }, async () => {
  resetTrackedWindowState(1);
  setTrackedTabRecords({
    1: createTabRecordFixture(1, {
      videoDetails: { title: 'Video 1', remainingTime: 120, lengthSeconds: 120 },
